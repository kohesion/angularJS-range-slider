		    ...
		    scope.promiseInit.then(function () {
		        setTimeout(function () {
		            ...
		            var adjustHighSlidePosition = function (high) {
		                min = parseFloat(scope.min);
		                max = parseFloat(scope.max);
		                (high < low) && (scope.low = low = high * .95); // && (node[0].querySelector('.low').style.left = Math.max((low - min) / (max - min) * scope.width, scope.min) + 'px');
		                var elem = node[0].querySelector('.high');
		                var left = scope.dragHandleHigh.convertValueToPixels(high);
		                //var left = parseInt(Math.min((high - min) / (max - min) * scope.width, scope.width));
		                scope.leftHigh = left;
